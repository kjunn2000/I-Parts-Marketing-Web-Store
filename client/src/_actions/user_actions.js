...
export function getUserId(emailData){
    
    const request = axios.post(`/api/users/getUserId`,emailData)
        .then(response => response.data);

    return {
        type: GET_USERID_USER,
        payload: request
    }
}

export function sentLink(dataToSubmit){
    
    const request = axios.post(`${USER_SERVER}/reset`,dataToSubmit)
        .then(response => response.data);
